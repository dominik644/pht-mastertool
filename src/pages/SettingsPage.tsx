...
const tabs = [
  ...
  { id: 'users' as const, label: 'Zugänge', adminOnly: true as const },
];
...
export function SettingsPage() {
  ...
  const activeTab = searchParams.get('tab') === 'users' && admin ? 'users' : 'general';
  ...
  return (
    <div className={`${isMobileView ? 'p-4' : 'p-6 lg:p-8'} max-w-4xl mx-auto`}>
      ...
      <div className="flex gap-1 mb-6 border-b border-dark-500/50">
        {tabs.filter((t) => admin || !('adminOnly' in t)).map(({ id, label }) => (
          <button
            key={id}
            type="button"
            onClick={() => setTab(id)}
          ...
          {admin && (
            <>
              ...
              <div className="space-y-3">
                {adminLinks.map(({ to, label, desc, icon: Icon }) => (
                  <Link key={to} to={to}>
