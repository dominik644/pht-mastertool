...
export function stripCachedScores(raw: GlobalTenderRaw): GlobalTenderRaw {
...
export function filterReprocessCandidates(stored: Tender[]): Tender[] {
  return stored.filter((t) => !isDemoTenderId(t.id) && isTenderStillActive(t));
}
