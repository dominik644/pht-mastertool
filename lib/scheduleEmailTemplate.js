  ...
  return `
    <tr>
      <td style="padding:0;${emailHeroBackgroundStyle()}">
        <div style="padding:32px 28px 28px;max-width:320px">
...
<body style="font-family:'Segoe UI',Inter,Arial,sans-serif;background:${PHT_COLORS.bg};color:${PHT_COLORS.text};margin:0;min-height:100vh;padding:24px 12px">
  <div style="max-width:580px;width:100%;margin:0 auto;background:${PHT_COLORS.card};border:1px solid ${PHT_COLORS.cardBorder};border-radius:4px;overflow:hidden;box-shadow:0 4px 24px rgba(23,65,125,0.08)">
...
function slotCardsHtml(slotCards) {
  return slotCards
    .map(
      (s, i) => `
        <tr>
          <td style="padding:${i === 0 ? '0' : '8px'} 0 0">
            <a href="${escapeHtml(s.url)}" style="display:block;text-decoration:none;background:${PHT_COLORS.accent};border-radius:6px;padding:14px 20px;text-align:center">
              ${s.weekday ? `<div style="font-size:10px;color:rgba(255,255,255,0.85);font-weight:600;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:3px">${escapeHtml(s.weekday)}</div>` : ''}
              <div style="font-size:15px;font-weight:600;color:${PHT_COLORS.white}">${escapeHtml(s.label)}</div>
              <div style="font-size:11px;color:rgba(255,255,255,0.8);margin-top:4px">45 Min. · Besuch vor Ort · Termin wählen →</div>
            </a>
          </td>
        </tr>`,
    )
...
<body style="margin:0;padding:0;background:${PHT_COLORS.bg};font-family:'Segoe UI',Inter,Arial,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${PHT_COLORS.bg};padding:24px 12px">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;background:${PHT_COLORS.card};border:1px solid ${PHT_COLORS.cardBorder};border-radius:4px;overflow:hidden;box-shadow:0 4px 24px rgba(23,65,125,0.08)">
          ${phtWhiteHeaderBar()}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .card { max-width: 580px; width: 100%; background: ${PHT_COLORS.card}; border: 1px solid ${PHT_COLORS.cardBorder}; border-radius: 4px; overflow: hidden; box-shadow: 0 4px 24px rgba(23,65,125,0.08); }
