...
function phtLogoBlock() {
  return `
    <div style="display:inline-block;background:${PHT_COLORS.navy};border-radius:8px;padding:10px 18px;margin-bottom:10px">
      <span style="color:${PHT_COLORS.white};font-weight:700;font-size:22px;letter-spacing:0.12em">PHT</span>
    </div>`;
}
...
function phtHeader() {
  return `
    <tr>
      <td style="padding:0;background:${PHT_COLORS.navy};text-align:center">
        <div style="padding:28px 32px 24px">
          ${phtLogoBlock()}
          <div style="font-size:12px;font-weight:500;color:rgba(255,255,255,0.85);letter-spacing:0.04em;margin-top:6px">Planungsbüro für Hygiene-Technik</div>
        </div>
      </td>
    </tr>`;
}
...
function phtFooter() {
  return `
    <tr>
      <td style="padding:24px 32px 28px;border-top:1px solid ${PHT_COLORS.cardBorder};background:${PHT_COLORS.cardMuted};text-align:center">
...
export function buildBrandedPage({ title, bodyHtml, variant = 'default' }) {
  const accent = variant === 'error' ? PHT_COLORS.error : variant === 'success' ? PHT_COLORS.navy : PHT_COLORS.navy;
...
<body style="font-family:'Segoe UI',Inter,Arial,sans-serif;background:${PHT_COLORS.bg};color:${PHT_COLORS.text};margin:0;min-height:100vh;padding:24px">
  <div style="max-width:560px;width:100%;margin:0 auto;background:${PHT_COLORS.card};border:1px solid ${PHT_COLORS.cardBorder};border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(23,65,125,0.08)">
    <div style="padding:24px 32px;background:${PHT_COLORS.navy};text-align:center">
      ${phtLogoBlock()}
    </div>
    <div style="padding:32px;text-align:center">
      <div style="width:52px;height:52px;border-radius:50%;background:${accent}15;border:2px solid ${accent};margin:0 auto 20px;line-height:48px;font-size:22px;color:${accent};font-weight:700">${icon}</div>
    ...
    <div style="padding:20px 32px;border-top:1px solid ${PHT_COLORS.cardBorder};background:${PHT_COLORS.cardMuted};text-align:center">
      <a href="https://pht.group" style="${linkStyle()};font-size:13px">pht.group</a>
      ...
      (s) => `
        <tr>
          <td style="padding:6px 0">
            <a href="${escapeHtml(s.url)}" style="display:block;text-decoration:none;background:${PHT_COLORS.card};border:1px solid ${PHT_COLORS.cardBorder};border-radius:10px;padding:16px 20px;border-left:4px solid ${PHT_COLORS.accent}">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    ${s.weekday ? `<div style="font-size:11px;color:${PHT_COLORS.navy};font-weight:600;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:4px">${escapeHtml(s.weekday)}</div>` : ''}
                    <div style="font-size:16px;font-weight:600;color:${PHT_COLORS.text}">${escapeHtml(s.label)}</div>
                  ...
                  <td width="36" style="text-align:right;vertical-align:middle">
                    <div style="width:32px;height:32px;border-radius:8px;background:${PHT_COLORS.accent};color:#fff;font-size:18px;line-height:32px;text-align:center">→</div>
                  </td>
                </tr>
              </table>
            </a>
...
export function injectCustomMessageHtml(html, customMessage) {
  ...
  const anchor = '<p style="margin:20px 0 12px';
...
<body style="margin:0;padding:0;background:${PHT_COLORS.bg};font-family:'Segoe UI',Inter,Arial,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${PHT_COLORS.bg};padding:24px 12px">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:580px;background:${PHT_COLORS.card};border:1px solid ${PHT_COLORS.cardBorder};border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(23,65,125,0.08)">
          ${phtHeader()}
          <tr>
            <td style="padding:28px 32px;color:${PHT_COLORS.text};line-height:1.65;font-size:15px">
              ...
              <p style="margin:20px 0 12px;color:${PHT_COLORS.text};font-weight:600;font-size:15px">Ihre Terminvorschläge</p>
...
function wishLinkHtml(wishUrl) {
  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:16px">
      <tr>
        <td style="padding:6px 0">
          <a href="${escapeHtml(wishUrl)}" style="display:block;text-decoration:none;background:${PHT_COLORS.cardMuted};border:1px dashed ${PHT_COLORS.navy};border-radius:10px;padding:16px 20px">
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .card { max-width: 540px; width: 100%; background: ${PHT_COLORS.card}; border: 1px solid ${PHT_COLORS.cardBorder}; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(23,65,125,0.08); }
    .header { padding: 24px 32px; text-align: center; background: ${PHT_COLORS.navy}; }
    .logo { display: inline-block; background: transparent; color: ${PHT_COLORS.white}; font-weight: 700; font-size: 22px; letter-spacing: 0.12em; }
    .brand-sub { font-size: 12px; color: rgba(255,255,255,0.85); margin-top: 6px; letter-spacing: 0.04em; }
    .content { padding: 28px 32px; }
    h1 { font-size: 1.35rem; margin: 0 0 8px; color: ${PHT_COLORS.text}; font-weight: 300; }
    ...
    button[type=submit] { width: 100%; padding: 14px; border: none; border-radius: 8px; background: ${PHT_COLORS.accent}; color: #fff; font-size: 15px; font-weight: 600; cursor: pointer; margin-top: 8px; }
    button[type=submit]:hover { background: ${PHT_COLORS.accentDark}; }
    .footer { padding: 20px 32px; border-top: 1px solid ${PHT_COLORS.cardBorder}; background: ${PHT_COLORS.cardMuted}; text-align: center; font-size: 11px; color: ${PHT_COLORS.dim}; }
...
<body>
  <div class="card">
    <div class="header">
      <div class="logo">PHT</div>
      <div class="brand-sub">Planungsbüro für Hygiene-Technik</div>
    </div>
    <div class="content">
    ...
    <div class="footer">
      <a href="https://pht.group">pht.group</a> · Hygienelösungen für die Lebensmittelindustrie
    </div>
  </div>
  <script>
