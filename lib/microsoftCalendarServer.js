...
/**
 * @param {{ subject: string, body: string, start: string, end: string, location?: string, attendeeEmails?: string[], calendarUser?: string }} params
 */
export async function createServerCalendarEvent(params) {
  const token = await getAppAccessToken();
  const user = params.calendarUser || getCalendarUser();
  if (!token || !user) {
