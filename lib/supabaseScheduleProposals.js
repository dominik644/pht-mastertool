import {
  ...
  listFileScheduleProposalsByStatuses,
 ...
 */
export async function fetchScheduleProposalsByStatuses(statuses) {
  const list = Array.isArray(statuses) ? statuses.filter(Boolean) : [];
  if (list.length === 0) return { ok: true, proposals: [] };
  if (list.length === 1) return fetchScheduleProposalsByStatus(list[0]);
  ...
  return withStorageFallback(
    ...
    () => listFileScheduleProposalsByStatuses(list),
  );
}
