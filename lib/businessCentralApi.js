...
export async function fetchSalesQuotesByCustomer(customerNo) {
  ...
  const path = `/salesQuotes?$filter=customerNumber eq '${no}'&$select=id,number,customerNumber,customerName,orderDate,dueDate,totalAmountIncludingTax,currencyCode,status&$orderby=orderDate desc&$top=50`;
...
export async function fetchSalesInvoicesByCustomer(customerNo) {
  const no = encodeURIComponent(String(customerNo).trim());
  const path = `/salesInvoices?$filter=customerNumber eq '${no}'&$select=id,number,customerNumber,customerName,invoiceDate,dueDate,totalAmountIncludingTax,currencyCode,status&$orderby=invoiceDate desc&$top=50`;
  return fetchAllPages(path);
}
...
export async function fetchBcDocumentsByCustomer(customerNo, type) {
  ...
  throw new Error('type muss quote oder invoice sein');
}
