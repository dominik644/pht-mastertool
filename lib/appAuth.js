...
import { defaultPasswordHash, nameToUsername } from './defaultAppUsers.js';
...
/** @param {{ email: string, username?: string, name?: string, admin?: boolean, bcSalespersonCode?: string, salesRep?: string, disabled?: boolean, mustChangePassword?: boolean, passwordHash?: string, password?: string }} match */
export function buildUserProfile(match) {
  if (!match || match.disabled) return null;
  const admin = isAdminUser(match);
  const storedHash = match.passwordHash ?? match.password;
  const stillOnInitialPassword = storedHash && storedHash === defaultPasswordHash();
  const mustChangePassword = match.mustChangePassword === true && stillOnInitialPassword;
  return {
    email: match.email,
...
export function createSessionToken(email, { mustChangePassword = false } = {}) {
...
export function verifySessionToken(token) {
  ...
  try {
    ...
    return {
      ...
      hasMustChangePasswordFlag: Object.prototype.hasOwnProperty.call(payload, 'mcp'),
    };
  } catch {
    return null;
  }
