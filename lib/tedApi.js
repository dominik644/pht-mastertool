...
/** TED v3 drosselt parallele Requests – sequentielle Batches vermeiden leere Antworten. */
const TED_FETCH_CONCURRENCY = 8;
...
export async function fetchTEDNotices(options = {}) {
  ...
  for (let i = 0; i < queryPlan.length; i += TED_FETCH_CONCURRENCY) {
    const chunk = queryPlan.slice(i, i + TED_FETCH_CONCURRENCY);
    const chunkResults = await Promise.allSettled(
      chunk.map(({ query, limit: perQuery }) => fetchTEDNoticesPage(query, perQuery, scope)),
    );
