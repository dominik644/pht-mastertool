...
/** TED v3 – praktisches Seitenlimit (API erlaubt bis 1000). */
export const TED_FETCH_LIMIT = 800;

/** Kern-Hygiene-Queries vor Länder-Breitband (Budget-Cap für TED_COUNTRY_QUERIES). */
const TED_COUNTRY_QUERY_CAP = 22;

/** Food-Facility-Queries separat mit festem Limit (kritisch für Lebensmittelbetrieb-Ingest). */
const FOOD_FACILITY_PER_QUERY_LIMIT = 15;

function allocateTEDQueryLimits(totalLimit) {
  const coreQueries = [
    ...TED_SEARCH_QUERIES,
    ...TED_CPV_QUERIES,
    ...TED_DE_CPV_QUERIES,
  ...
  const countryQueries = [
    ...TED_COUNTRY_QUERIES.slice(0, TED_COUNTRY_QUERY_CAP),
  ];
  const coreBudget = Math.floor(totalLimit * 0.72);
  const countryBudget = totalLimit - coreBudget;
  return [
    ...coreQueries.map((query) => ({
      query,
      limit: Math.max(12, Math.ceil(coreBudget / coreQueries.length)),
    })),
    ...countryQueries.map((query) => ({
      query,
      limit: Math.max(6, Math.ceil(countryBudget / countryQueries.length)),
    })),
  ];
}
...
export async function fetchTEDNotices(options = {}) {
  ...
  const queryPlan = allocateTEDQueryLimits(limit);
  const foodQueries = [
    ...TED_FOOD_FACILITY_QUERIES,
    ...TED_FOOD_FACILITY_COUNTRY_QUERIES,
  ];

  const [foodBatches, mainBatches] = await Promise.all([
    Promise.allSettled(
      foodQueries.map((query) => fetchTEDNoticesPage(query, FOOD_FACILITY_PER_QUERY_LIMIT, scope)),
    ),
    Promise.allSettled(
      queryPlan.map(({ query, limit: perQuery }) => fetchTEDNoticesPage(query, perQuery, scope)),
    ),
  ]);

  const batches = [...foodBatches, ...mainBatches];
...
export async function loadTendersFromTED() {
  try {
    const { notices, total } = await fetchTEDNotices({ limit: TED_FETCH_LIMIT });
