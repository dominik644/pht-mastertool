...
/** Mehrere kurze Queries – lange ACTIVE-Queries liefern bei TED oft 0 Treffer */
...
export async function fetchTEDNotices(options = {}) {
  ...
  const perQuery = Math.max(15, Math.ceil(limit / TED_SEARCH_QUERIES.length));

  const batches = await Promise.allSettled(
    TED_SEARCH_QUERIES.map((query) => fetchTEDNoticesPage(query, perQuery, scope)),
  );
