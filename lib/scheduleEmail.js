...
async function sendViaResend({ to, subject, html, text, ics }) {
  ...
  if (attachment) {
    payload.attachments = [{ filename: attachment.name, content: attachment.contentBytes }];
  }
...
export async function sendScheduleEmail(params) {
  ...
  if (hasServerMailConfig()) {
    const attachment = icsAttachment(params.ics);
 ...
 */
export function buildSalesNotificationEmail({ customerName, slot, customerEmail, calendarOk }) {
  const when = formatSlotGerman(slot);
  const calLine = calendarOk
    ? '<p>Der Termin liegt in deinem Outlook-Kalender. Zusätzlich ist eine .ics angehängt.</p>'
    : '<p>Bitte die angehängte .ics-Datei in Outlook öffnen — der Serverkalender war nicht erreichbar.</p>';
  return {
    subject: `[PHT] Kunde hat Termin gewählt: ${customerName}`,
    html: `<p><strong>${escapeHtml(customerName)}</strong> (${escapeHtml(customerEmail)}) hat einen Besuchstermin bestätigt:</p><p><strong>${escapeHtml(when)}</strong></p>${calLine}`,
