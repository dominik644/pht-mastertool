...
export function buildMailtoBody({ text, html, slotOptions, wishUrl }) {
  if (slotOptions?.length) {
    const wishLine = wishUrl ? ['', 'Keiner passt? Eigenen Wunschtermin angeben:', wishUrl] : [];
    return [
...
export function buildProposalEmail(ctx) {
  ...
  const introExtra = regionalNote
    ? `<p style="margin:0 0 16px;padding:12px 16px;background:${PHT_COLORS.cardMuted};border-left:4px solid ${PHT_COLORS.navy};border-radius:0 8px 8px 0;color:${PHT_COLORS.muted};font-size:14px">${escapeHtml(regionalNote)}</p>`
    : '';
  ...
  const slotOptions = slotCards.map(({ label, url, weekday }) => ({ label: weekday ? `${weekday}, ${label}` : label, url }));
  if (wishUrl) {
