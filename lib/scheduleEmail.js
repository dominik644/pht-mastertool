...
export function buildProposalEmail(ctx) {
  ...
  const regionalHtml = regionalNote ? `<p>${escapeHtml(regionalNote)}</p>` : '';
  const regionalText = regionalNote ? `\n\n${regionalNote}\n` : '';
...
gerne möchten wir einen persönlichen Besuch bei ${customerName} vereinbaren.${regionalText}
