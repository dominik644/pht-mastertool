...
import { collectOcdsCpvCodes } from './cpvMatch.js';
...
function collectCpvCodes(tender) {
  return collectOcdsCpvCodes(tender);
}
 ...
 */
/** pubDay-Export unterstützt historische Tage (getestet bis 14 Tage). */
export const OEFFENTLICHEVERGABE_MAX_DAYS = 14;
export const OEFFENTLICHEVERGABE_DEFAULT_DAYS = 7;

export async function fetchOeffentlichevergabeTenders(options = {}) {
  const days = Math.min(Math.max(options.days ?? OEFFENTLICHEVERGABE_DEFAULT_DAYS, 1), OEFFENTLICHEVERGABE_MAX_DAYS);
  const dayStrings = [];
