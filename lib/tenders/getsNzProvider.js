/**
 * New Zealand GETS – MBIE Open Data CSV (Award Notices, kein Live-REST)
 * Tail-Fetch der letzten ~1.5 MB (Range) – 20 MB Vollfile zu groß für Serverless.
 * https://www.mbie.govt.nz/.../open-data → GETS-award-notices.csv
 */
...
function mapRow(headers, values) {
  ...
  return {
    ...
    description: `${title}. Agency: ${rec['Posting Agency'] || ''}. Type: ${rec['RFx Type'] || ''}.`.slice(0, 800),
...
export async function fetchGetsNzTenders() {
  try {
    ...
    const tenders = rows
      .slice(-MAX_ROWS)
      .map((values) => mapRow(CSV_HEADERS, values))
      .filter((t) => {
    ...
    return { tenders: unique, source: 'gets-nz', live: unique.length > 0 };
  } catch (err) {
