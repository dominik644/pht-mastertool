 ...
 * @returns {Promise<{ tenders: object[], source: string, regions: string[], total: number, error?: string, isDemo: boolean, liveProviders: string[], providerCount: number, lastBulkUpdate?: string, bulkFreshnessLabel?: string, bulkStale?: boolean }>}
 */
export async function loadAllTenders(options = {}) {
  const mobile = options.mobile === true;
  const activeProviders = mobile
  ...
  results.forEach((result, i) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      if (bulkStaleWarning) bulkWarnings.push(bulkStaleWarning);
      if (batch.length) {
        tenders.push(...batch);
  ...
  const bulkFreshnessLabel = formatBulkFreshnessDe(bulkMeta.lastBulkUpdate);
  const allErrors = [...errors, ...bulkWarnings].filter(Boolean);
