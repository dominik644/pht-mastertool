...
import { collectOcdsCpvCodes, cpvMatchesPHT } from './cpvMatch.js';
import {
...
export function matchesPHTText(text, cpvCodes = []) {
  ...
  if (hasCoreProductSignal(lower)) return true;
  if (cpvMatchesPHT(cpvCodes)) return true;
  ...
  if (onlyWeakContext && !hasStrongHygieneContext(lower) && !hasCoreProductSignal(lower)) {
    return false;
  }
  return true;
}
