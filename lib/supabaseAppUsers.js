...
function formatSupabaseError(raw) {
  ...
  if (/PGRST205|could not find the table.*app_users|schema cache/i.test(text)) {
    return 'Supabase-Tabelle app_users fehlt. Im SQL Editor supabase/app_users.sql ausführen.';
  }
  ...
  try {
    ...
    if (/PGRST205|could not find the table.*app_users|schema cache/i.test(msgStr)) {
      return 'Supabase-Tabelle app_users fehlt. Im SQL Editor supabase/app_users.sql ausführen.';
    }
    if (/PGRST125|invalid path specified/i.test(msgStr)) {
      return 'Supabase-URL falsch: SUPABASE_URL nur als Projektdomain (https://xxx.supabase.co), ohne /rest/v1';
    }
    return msgStr.slice(0, 200);
  } catch {
    return text.slice(0, 160);
  }
}

function isAppUsersTableMissing(raw) {
  return /PGRST205|could not find the table.*app_users|schema cache/i.test(String(raw ?? ''));
}

function supabaseFailure(raw) {
  return { error: formatSupabaseError(raw), tableMissing: isAppUsersTableMissing(raw) };
}
...
export async function createDbUser(input) {
  ...
  if (!res.ok) {
    ...
    return { ok: false, ...supabaseFailure(err) };
  }
  return { ok: true };
}
