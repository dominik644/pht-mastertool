...
/** @returns {Promise<{ email: string, passwordHash: string, name?: string, admin: boolean, disabled: boolean }[]>} */
export async function fetchDbUsers() {
  const client = getClient();
  if (!client) return [];
  const res = await fetch(`${client.url}/rest/v1/app_users?select=email,name,password_hash,admin,disabled`, {
    headers: {
      apikey: client.key,
...
/** @returns {Promise<{ email: string, name?: string, admin: boolean, disabled: boolean, source: string }[]>} */
...
/** @param {{ email: string, password: string, name?: string, admin?: boolean }} input */
...
/** @param {{ email: string, disabled?: boolean, admin?: boolean, password?: string }} input */
