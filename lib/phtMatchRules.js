...
/**
 * Bau/Umbau/Sanierung – kein PHT-Geschäft ohne Ausrüstungssignal.
 * PHT liefert Reinigungsgeräte, nicht Umbau von Lebensmittelbetrieben o. Ä.
 */
export const PHT_CONSTRUCTION_RENOVATION_KEYWORDS = [
  'umbau', 'umbauarbeiten', 'umbau von', 'umbau des', 'umbau der',
  'bauarbeiten', 'bauleistungen', 'baumaßnahmen', 'baumassnahmen', 'baumaßnahme',
  'neubau', 'rohbau', 'ausbau', 'sanierung', 'renovation', 'renovierung', 'renovate',
  'construction', 'building works', 'civil works', 'refurbishment', 'refurbish',
  'generalunternehmer', 'generalübernehmer', 'generaluebernehmer',
  'abbruch', 'abbrucharbeiten', 'erdarbeiten', 'mauerarbeiten', 'hochbau', 'tiefbau',
  'hallenneubau', 'produktionshalle', 'food plant renovation', 'food facility renovation',
  'food processing facility renovation', 'food industry renovation', 'food industry construction',
];

/** Lebensmittelbetrieb als Bau-/Umbau-Kontext – ohne Gerätebezug kein PHT-Match. */
export const PHT_FOOD_FACILITY_CONTEXT_KEYWORDS = [
  'lebensmittelbetrieb', 'lebensmittelbetriebe', 'food facility', 'food plant',
  'food processing facility', 'food processing plant', 'meat processing plant',
  'dairy plant', 'slaughterhouse renovation', 'schlachthof umbau',
];
...
/**
 * Umbau/Bau/Sanierung von Betrieben (v. a. Lebensmittel) – ohne Ausrüstungssignal ausschließen.
 * Reinigungsanlagen für Lebensmittelbetriebe bleiben erlaubt (hasStrongEquipmentSignal).
 */
export function textHasConstructionRenovationExclusion(text) {
  const lower = String(text || '').toLowerCase();
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;

  const hasConstruction = PHT_CONSTRUCTION_RENOVATION_KEYWORDS.some((kw) => lower.includes(kw));
  const hasFoodFacility = PHT_FOOD_FACILITY_CONTEXT_KEYWORDS.some((kw) => lower.includes(kw));

  if (hasConstruction) return true;
  if (hasFoodFacility) return true;

  const foodIndustryRenovation = (
    (lower.includes('food') || lower.includes('lebensmittel'))
    && (lower.includes('renovation') || lower.includes('construction') || lower.includes('umbau')
      || lower.includes('bauarbeit') || lower.includes('sanierung'))
  );
  return foodIndustryRenovation;
}
...
export function textHasNonPHTServiceExclusion(text) {
  ...
  if (textHasConstructionRenovationExclusion(text)) return true;
  if (isGenericProcurementOnly(text)) return true;
  if (isDpsWithoutEquipment(text)) return true;
  return false;
}

/** Score ≥70 (GO) nur mit starkem Ausrüstungs-, CPV- oder Preislisten-Signal. */
export function qualifiesForGoScore(text, cpvCodes = [], priceListArticleMatch = false, cpvEquipmentFn = null) {
