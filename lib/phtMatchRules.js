...
export const PHT_GENERIC_CONSTRUCTION_KEYWORDS = [
  ...
  'erdarbeiten', 'mauerarbeiten', 'maurerarbeiten', 'betonarbeiten', 'abbrucharbeiten', 'abbruch',
  'ausbauarbeiten', 'hochbauarbeiten', 'tiefbauarbeiten', 'rohbauarbeiten', 'putzarbeiten',
  'building works', 'civil works', 'construction work', 'construction of', 'construction works',
  'earthworks', 'masonry work', 'concrete work', 'shell construction', 'general contractor',
...
/**
 * Hygieneplanung / Beratung – kein PHT-Geschäft (PHT liefert Anlagen, keine Planungsdienstleistung).
 * Ausnahme: explizites Geräte-/Anlagensignal (Hygienestation, Waschanlage etc.).
 */
export const PHT_HYGIENE_PLANNING_KEYWORDS = [
  'hygieneplanung', 'hygiene planning', 'hygiene concept consulting', 'hygiene consulting',
  'hygieneberatung', 'hygieneberatungs', 'hygieneberatungsleistung', 'hygieneberatungsleistungen',
  'haccp-beratung', 'haccp beratung', 'haccp consulting', 'haccp consultancy',
  'hygienekonzept', 'hygienekonzeption', 'hygiene concept', 'hygiene concept development',
  'erstellung hygienekonzept', 'erstellung eines hygienekonzepts', 'hygiene audit',
  'hygiene-audit', 'hygiene auditing', 'gmp beratung', 'gmp consulting', 'gmp consultancy',
  'hygiene consulting services', 'hygiene plan development', 'hygieneplaner',
  'planung hygienekonzept', 'hygiene concept planning', 'conseil hygiène', 'audit hygiène',
];

/**
 * Abwasser-/Kläranlagen – kein PHT-Geschäft (auch nicht „im Freien“ / outdoor).
 * PHT liefert Industriewaschanlagen (Kisten, IBC, Paletten) – keine Abwassertechnik.
 */
export const PHT_WASTEWATER_KEYWORDS = [
  'abwasser', 'abwässer', 'abwaesser', 'wastewater', 'waste water', 'sewage',
  'kläranlage', 'klaeranlage', 'klärwerk', 'klaerwerk', 'abwasserbehandlung',
  'abwassertechnik', 'abwasseranlage', 'abwasserreinigung', 'abwasserentsorgung',
  'klärschlamm', 'klaerschlamm', 'effluent treatment', 'sewage treatment',
  'wastewater treatment', 'wastewater plant', 'sewage plant', 'sewage works',
  'wastewater works', 'water treatment plant', 'wwtp', 'stp',
  'ara ', ' ara', 'abwasserreinigungsanlage',
  'schlammentwässerung', 'schlammentwaesserung', 'regenwasserbehandlung',
  'klärbecken', 'klaerbecken', 'faulbecken', 'klärschlammverbrennung',
  'stationäre kläranlage', 'stationaere klaeranlage',
  'traitement des eaux usées', 'station d\'épuration', 'station depuration',
  'wastewater engineering', 'sewerage', 'kanalisation',
];

/** Outdoor-Kontext bei Abwasser – zusätzliches Ausschlusssignal. */
const PHT_OUTDOOR_CONTEXT_PHRASES = [
  'im freien', 'outdoor', 'außenbereich', 'aussenbereich', 'open air', 'open-air',
  'freiluft', 'outside installation', 'external installation',
];

/**
 * Generische Dienstleistungen ohne Ausrüstungsbezug – kein PHT-Geschäft.
 * Ausnahme: explizites Geräte-/Anlagensignal.
 */
export const PHT_GENERIC_SERVICE_KEYWORDS = [
  'facility management', 'gebäudemanagement', 'gebaeudemanagement', 'objektmanagement',
  'fm-dienstleistung', 'fm dienstleistung', 'fm-vertrag', 'fm vertrag',
  'facility services', 'building management services', 'property management services',
  'beratungsleistung', 'beratungsleistungen', 'beratungsdienstleistung', 'beratungsdienstleistungen',
  'unternehmensberatung', 'management consulting', 'consulting services', 'advisory services',
  'consultancy services', 'professional services contract', 'beratungsvertrag',
  'wartungsvertrag', 'instandhaltungsvertrag', 'maintenance contract', 'maintenance services contract',
  'wartungsdienstleistung', 'wartungsdienstleistungen', 'instandhaltungsdienstleistung',
  'instandhaltungsleistung', 'instandhaltungsleistungen', 'service level agreement',
  'rahmenvertrag dienstleistung', 'rahmenvertrag dienstleistungen',
  'dienstleistungsvertrag', 'dienstleistungsrahmenvertrag',
  'reinigungsdienst', 'reinigungsdienste',
  'prestation de services', 'services contract', 'service contract',
...
/** Abwasser-/Kläranlagen – kein PHT ohne Industriewaschanlagen-Signal. */
export function textHasWastewaterExclusion(text) {
  const lower = String(text || '').toLowerCase();
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;

  if (PHT_WASTEWATER_KEYWORDS.some((kw) => lower.includes(kw))) return true;
  if (/\bara\b/.test(lower) && (lower.includes('klär') || lower.includes('klaer') || lower.includes('abwasser'))) {
    return true;
  }
  const outdoorWastewater = PHT_OUTDOOR_CONTEXT_PHRASES.some((kw) => lower.includes(kw))
    && (lower.includes('abwasser') || lower.includes('wastewater') || lower.includes('sewage'));
  return outdoorWastewater;
}

/** Hygieneplanung / HACCP-Beratung – ohne Anlagenbezug ausschließen. */
export function textHasHygienePlanningExclusion(text) {
  const lower = String(text || '').toLowerCase();
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;
  if (hasEquipmentSignal(lower)) return false;
  return PHT_HYGIENE_PLANNING_KEYWORDS.some((kw) => lower.includes(kw));
}

/** CPV 45xxxx (Bauarbeiten) ohne Geräte-CPV – reines Baugeschäft. */
export function cpvIndicatesPureConstruction(cpvCodes = []) {
  const codes = (cpvCodes || [])
    .map((c) => String(c).replace(/\D/g, '').slice(0, 8))
    .filter((c) => c.length >= 2);
  if (!codes.length) return false;
  const constructionCodes = codes.filter((c) => c.startsWith('45') && !c.startsWith('4533'));
  if (!constructionCodes.length) return false;
  const equipmentPrefixes = ['4292', '4299', '3971', '4461', '3319', '3379', '3983', '3913', '4442'];
  const hasEquipmentCpv = codes.some((c) => equipmentPrefixes.some((p) => c.startsWith(p)));
  return !hasEquipmentCpv;
}

export function textHasConstructionCpvExclusion(text, cpvCodes = []) {
  if (!cpvIndicatesPureConstruction(cpvCodes)) return false;
  if (hasStrongEquipmentSignal(text)) return false;
  if (hasEquipmentSignal(text)) return false;
  return true;
}

/** FM, Beratung, Wartungsverträge etc. – ohne Geräte ausschließen. */
export function textHasGenericServiceExclusion(text) {
  const lower = String(text || '').toLowerCase();
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;
  if (hasEquipmentSignal(lower)) return false;

  if (PHT_GENERIC_SERVICE_KEYWORDS.some((kw) => lower.includes(kw))) return true;

  const bareConsulting = /\b(beratung|consulting|consultancy)\b/.test(lower);
  const bareFm = /\bfacility\s+management\b/.test(lower) || /\bgebäudemanagement\b/.test(lower)
    || /\bgebaeudemanagement\b/.test(lower);
  if ((bareConsulting || bareFm) && !hasEquipmentSignal(lower)) return true;
...
/**
 * Generisches Bauen/Sanieren ohne Gerätebezug ausschließen.
 * Lebensmittelbetrieb-Neubau mit Waschanlage/Schaumstation bleibt über Gerätesignal erlaubt.
 */
export function textHasGenericConstructionExclusion(text, cpvCodes = []) {
  const lower = String(text || '').toLowerCase();
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;
  if (hasEquipmentSignal(lower)) return false;
  ...
  if (textHasConstructionCpvExclusion(lower, cpvCodes)) return true;
  ...
  const bareAusbau = /\bausbau\b/.test(lower) && !hasEquipmentSignal(lower);
  if (bareAusbau) return true;
...
export function textHasNonPHTServiceExclusion(text, cpvCodes = []) {
  if (textHasWastewaterExclusion(text)) return true;
  if (textHasHygienePlanningExclusion(text)) return true;
  if (textHasGenericServiceExclusion(text)) return true;
  ...
  if (textHasGenericConstructionExclusion(text, cpvCodes)) return true;
 ...
 */
export function isPureGenericService(text) {
  return textHasGenericServiceExclusion(text);
}
...
export function isPureCleaningService(text) {
  ...
  if (!lower.trim()) return false;
  if (hasStrongEquipmentSignal(lower)) return false;
  if (isPureGenericService(lower)) return true;
...
export function hasStrongHygieneContext(text) {
  ...
  if (hasFoodFacilityOpportunitySignal(lower) && hasEquipmentSignal(lower)) return true;
  return PHT_STRONG_HYGIENE_KEYWORDS.some((kw) => lower.includes(kw));
}
...
export function isServiceOnlyCleaning(
  ...
  if (isPureGenericService(text)) return true;
