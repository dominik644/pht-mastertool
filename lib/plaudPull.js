...
import { fetchPlaudNoteRaw, insertPlaudNote } from './plaudInbox.js';
import { loadPlaudRefreshToken, savePlaudRefreshToken } from './supabasePlaudInbox.js';
import { actionItemsFromProcessed, noteNeedsRefresh, unwrapPlaudText } from './plaudText.js';

const API_BASE = 'https://platform.plaud.ai/developer/api';
const REFRESH_URL = `${API_BASE}/oauth/third-party/access-token/refresh`;
const SUMMARY_TYPES = ['auto_sum_note', 'sum_multi_note', 'consumer_note'];
const POLISH_TYPES = ['transaction_polish'];
const MAX_IMPORT = 8;
...
function pickBlock(list, types) {
  if (!Array.isArray(list) || !list.length) return null;
  for (const type of types) {
    const hit = list.find((item) => item?.data_type === type);
    if (hit) return hit;
  }
  return null;
}
...
function asFileList(payload) {
  ...
  const nested = payload?.detail ?? payload?.data;
  if (nested && typeof nested === 'object') {
    return Object.values(nested).filter((file) => file && typeof file === 'object' && file.id);
  }
  return [];
}
...
async function noteFromFile(file) {
  ...
  const summaryNote = pickBlock(noteList, SUMMARY_TYPES) || noteList[0];
  const summary = unwrapPlaudText(await loadBlockContent(summaryNote));
  const polish = pickBlock(sourceList, POLISH_TYPES);
  const polishText = polish ? unwrapPlaudText(await loadBlockContent(polish)) : '';
  const transcript = polishText && !isWrappedPlaudJson(polishText) && !polishText.startsWith('{')
    ? polishText
    : '';
  const actionItems = actionItemsFromProcessed(summary);
...
export async function syncPlaudFromAccount() {
  ...
  for (const item of files) {
    ...
    const existing = await fetchPlaudNoteRaw(noteId);
    if (existing && !noteNeedsRefresh(existing)) {
      skipped += 1;
      continue;
    }
    let detail;
    ...
    await insertPlaudNote(parsed, { replace: true });
    imported += 1;
  }
