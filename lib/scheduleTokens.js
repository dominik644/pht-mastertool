...
/** Token slot id for custom wish-appointment form access. */
export const WISH_SLOT_ID = 'wish';

/**
 * @param {{ proposalId: string, customerId: string, exp: number }} payload
 */
export function signWishToken({ proposalId, customerId, exp }) {
  return signScheduleToken({ proposalId, slotId: WISH_SLOT_ID, customerId, exp });
}
 ...
 * @returns {{ ok: true, payload: object } | { ok: false, error: string }}
 */
export function verifyWishToken(token) {
  const verified = verifyScheduleToken(token);
  if (!verified.ok) return verified;
  if (verified.payload.slotId !== WISH_SLOT_ID) {
    return { ok: false, error: 'Ungültiger Wunschtermin-Link' };
  }
  return verified;
}
