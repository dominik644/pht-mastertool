...
function getBaseUrl(req) {
  ...
  const host = (typeof forwarded === 'string' ? forwarded.split(',')[0].trim() : null)
    || req.headers?.host;
  if (host) {
...
export default async function handler(req, res) {
  ...
  if (!status.configured) {
    return res.status(503).json({
      ...status,
      skipped: true,
      error: 'Terminvorschläge benötigen Speicher (Datei oder Supabase) und SCHEDULE_TOKEN_SECRET',
    });
  }
  ...
  if (!insertResult.ok) {
    return res.status(insertResult.skipped ? 503 : 502).json({
      configured: true,
      storageMode: scheduleProposalStorageMode(),
      error: insertResult.error ?? 'Terminvorschlag konnte nicht gespeichert werden. Bitte erneut versuchen.',
    });
  }
  ...
  const signWish = () =>
    signWishToken({ proposalId, customerId, exp });
  ...
  if (sendResult.preview) {
    return res.status(200).json({
      ...
      emailPreview: {
        ...
        mailtoBody: sendResult.mailtoBody,
        mailtoUrl: sendResult.mailtoUrl,
      },
      message: 'Terminvorschläge bereit – Betreff und Nachricht bearbeiten, dann senden',
    });
  }
