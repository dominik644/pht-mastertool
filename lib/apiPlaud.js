...
import { guardSessionProfile } from './appAuthSession.js';
import {
  extractPlaudNote,
  fetchPlaudNote,
  insertPlaudNote,
...
export async function handlePlaud(req, res) {
  ...
  if (wantsAccount) {
    ...
    if (!guard.ok) return;
    const action = String(req.body?.action || req.query?.action || '').trim();
    if (action === 'connect' && !isAdminUser(guard.user)) {
      return res.status(403).json({ error: 'Nur Admin kann Plaud verbinden' });
    }
