 ...
 * @param {string} status
 */
export async function listFileScheduleProposalsByStatus(status) {
  if (!hasFileScheduleProposalStorage()) {
    return { ok: false, skipped: true };
  }
  const all = readAll();
  const proposals = Object.values(all).filter((p) => p.status === status);
 ...
 */
export async function listFileScheduleProposalsByStatuses(statuses) {
  if (!hasFileScheduleProposalStorage()) {
    return { ok: false, skipped: true };
  }
  const wanted = new Set(Array.isArray(statuses) ? statuses : []);
  const all = readAll();
  const proposals = Object.values(all).filter((p) => wanted.has(p.status));
  return { ok: true, proposals };
}
