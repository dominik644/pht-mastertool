...
/**
 * @param {{ name: string, contentType: string, contentBytes: string }} attachment
 */
function mapGraphAttachment(attachment) {
  return {
...
/**
 * @param {{ to: string, subject: string, body: string, html?: string, attachments?: { name: string, contentType: string, contentBytes: string }[] }} params
 */
export async function sendServerEmail({ to, subject, body, html, attachments }) {
  ...
  if (!token || !fromUser) {
    return {
      ok: false,
      skipped: true,
      error: 'Microsoft Graph Mail nicht verfügbar',
    };
  }
  ...
  if (attachments?.length) {
    message.attachments = attachments.map(mapGraphAttachment);
  }
