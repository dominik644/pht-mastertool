...
export function updateFileUserPassword(loginId, newPassword) {
  ...
  const writeTarget = resolveWritableDataFile();
  if (!writeTarget) {
    return { ok: true, user: users[idx], persisted: false };
  }
  if (!saveFileAppUsers(users)) {
    return { ok: false, error: 'Passwort konnte nicht gespeichert werden' };
  }
  return { ok: true, user: users[idx], persisted: true };
}

/** Mark first-login flag cleared without rotating password (e.g. after Supabase persist). */
export function clearFileUserMustChangePassword(loginId) {
  const users = loadFileAppUsers();
  const key = loginId.trim().toLowerCase();
  const compact = key.replace(/\s+/g, '');
  const idx = users.findIndex(
  ...
  if (idx < 0) return { ok: false, error: 'Benutzer nicht gefunden' };
  if (!users[idx].mustChangePassword) return { ok: true, user: users[idx], persisted: false };
  users[idx] = { ...users[idx], mustChangePassword: false };
  const writeTarget = resolveWritableDataFile();
  if (!writeTarget) return { ok: true, user: users[idx], persisted: false };
