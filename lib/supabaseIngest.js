...
/** Max tenders returned per paginated API request. */
export const MAX_API_PAGE_SIZE = 300;

/** Max rows scanned per API request (safety cap). */
const MAX_DB_SCAN = 12_000;

/** Max raw DB rows scanned for a full-list read. */
const SUPABASE_READ_LIMIT = 5000;

/** Raw rows fetched per Supabase round-trip while filling a page. */
const DB_BATCH_SIZE = 250;

/** In-memory cache for full-list reads (serverless warm instances). */
const ALL_READ_CACHE_TTL_MS = 5 * 60 * 1000;
/** @type {{ key: string, at: number, payload: object } | null} */
let allReadCache = null;
...
export async function fetchTendersFromSupabase(options = {}) {
  ...
  const limit = Math.min(Math.max(1, Math.floor(Number(options.limit) || DEFAULT_PAGE_SIZE)), MAX_API_PAGE_SIZE);
...
/**
 * Scan the full Supabase table once and return all PHT-relevant tenders.
 * Cached for 5 minutes per `since` key on warm server instances.
 *
 * @param {{ since?: string }} [options]
 */
export async function fetchAllTendersFromSupabase(options = {}) {
  const since = options.since;
  const cacheKey = since || '__all__';
  const now = Date.now();
  if (allReadCache && allReadCache.key === cacheKey && now - allReadCache.at < ALL_READ_CACHE_TTL_MS) {
    return { ok: true, ...allReadCache.payload, cached: true };
  }
  ...
  try {
    while (!dbExhausted && dbOffset < SUPABASE_READ_LIMIT) {
      const params = new URLSearchParams({
        select: 'id,title,country,deadline,url,source,raw_json,ingested_at',
        order: 'ingested_at.desc',
      ...
      for (const row of pageRows) {
        const scored = processTenderForRead(rowToTender(row));
        if (scored) pageTenders.push(scored);
      }

      dbOffset += pageRows.length;
      if (pageRows.length < DB_BATCH_SIZE) dbExhausted = true;
    }

    const payload = {
      tenders: pageTenders,
      total: pageTenders.length,
      estimatedDbTotal: dbTotal ?? undefined,
      page: 1,
      cursor: dbOffset,
      hasMore: false,
    };
    allReadCache = { key: cacheKey, at: now, payload };
    return { ok: true, ...payload };
  } catch (err) {
    return { ok: false, error: err.message || 'Supabase-Vollständig-Lesen fehlgeschlagen' };
  }
}
