...
/**
 * @param {{ since?: string, page?: number, limit?: number }} [options]
 * @returns {Promise<{ ok: boolean, tenders?: object[], total?: number, page?: number, hasMore?: boolean, skipped?: boolean, error?: string }>}
 */
export async function fetchTendersFromSupabase(options = {}) {
  ...
  try {
    while (filtered.length < neededEnd && !dbExhausted && dbOffset < MAX_DB_SCAN) {
      ...
      for (const row of pageRows) {
        ...
        if (scored) filtered.push(scored);
      }
    ...
    return { ok: true, tenders, total, page, hasMore };
  } catch (err) {
