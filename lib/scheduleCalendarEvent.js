...
export function resolveSalesNotifyEmail(proposal) {
  return (
    String(proposal?.sales_rep_email || '').trim()
    || process.env.SCHEDULE_SALES_NOTIFY_EMAIL?.trim()
    || process.env.INGEST_ALERT_EMAIL?.trim()
    || getCalendarUser()
    || 'weller@pht.group'
  );
}
...
/**
 * @param {{ customerName: string, customerEmail: string, slot: object, location?: string, calendarUser?: string }} ctx
 */
export async function createConfirmedVisitCalendarEvent(ctx) {
  ...
  const organizer = ctx.calendarUser || getCalendarUser();
  ...
  const attendeeEmails = [customerEmail].filter((email) => email && email.toLowerCase() !== String(organizer).toLowerCase());
  ...
  const ics = buildConfirmedVisitIcs({
    ...
    attendeeEmails: [organizer, ...attendeeEmails].filter(Boolean),
  });
