/**
 * PHT Power Scoring Engine – Score 0–100, Win-Fokus
 */
...
export function scoreTender(tender) {
  ...
  const score = Math.min(100, Math.round(keywordScore + budgetScore + regionScore + industryScore + cpvPts + dlPts));

  let recommendation;
  if (score >= 68) recommendation = 'GO';
  else if (score >= 38) recommendation = 'PRÜFEN';
  ...
  return {
    ...
    breakdown: {
      ...
      cpvScore: cpvPts, deadlineScore: dlPts, matchedKeywords,
    },
  };
}
