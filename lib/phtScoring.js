 ...
 */
import { PHT_CPV_CODES, PHT_CPV_EQUIPMENT_CODES, PHT_FOOD_FACILITY_SCORE_BOOST } from './phtConfig.js';
import { matchProductProfiles } from './productProfiles.js';
import { scorePriceListKeywords, hasPriceListArticleMatch } from './priceListKeywords.js';
import { scoreCatalogMatch } from './phtProductMatch.js';
import { isServiceOnlyCleaning, textHasExclusion, textHasNonPHTServiceExclusion, hasEquipmentSignal, hasFoodFacilityOpportunitySignal, qualifiesForGoScore } from './phtMatchRules.js';
import { matchPortfolioSegments, matchesAnyPortfolioSegment } from './phtPortfolio.js';
...
const KEYWORDS = {
  ...
  lebensmittelbetrieb: 10, lebensmittelbetriebe: 10, umbau: 6, zubau: 6, neubau: 6,
...
export function scoreTender(tender) {
  ...
  if (textHasExclusion(text)) {
    return {
      ...
      breakdown: {
        ...
        productProfileScore: 0, priceListScore: 0, catalogScore: 0, cpvScore: 0, deadlineScore: 0,
        matchedKeywords: [], priceListMatches: [], topCatalogArticles: [], topProfile: null, excluded: true,
      },
    };
  }

  if (textHasNonPHTServiceExclusion(text, tender.cpvCodes ?? [])) {
    return {
      score: 0,
      recommendation: 'NO-GO',
      category: categorizeBudget(tender.budgetEur ?? tender.budget ?? 0),
  ...
  if (!matchesAnyPortfolioSegment(text, cpvCodes) && !hasEquipmentSignal(text)) {
    return {
      score: 0,
      recommendation: 'NO-GO',
      category: categorizeBudget(tender.budgetEur ?? tender.budget ?? 0),
  ...
  const foodFacilityBoost = hasFoodFacilityOpportunitySignal(text) && hasEquipmentSignal(text)
    ? PHT_FOOD_FACILITY_SCORE_BOOST
    : 0;
  ...
  const priceListArticle = hasPriceListArticleMatch(text);
  ...
  const catalogHit = scoreCatalogMatch(text);
  const catalogScore = catalogHit.catalogScore;
  const catalogStrong = catalogHit.strongMatch;
  const portfolioHits = matchPortfolioSegments(text, cpvCodes);
  if (catalogHit.matchedLines[0]?.matchedKeywords) {
    for (const kw of catalogHit.matchedLines[0].matchedKeywords.slice(0, 4)) {
      if (!matchedKeywords.includes(kw)) matchedKeywords.push(kw);
    }
  }
  ...
  let score = Math.min(100, Math.round(
    keywordScore + foodFacilityBoost + budgetScore + regionScore + industryScore + cpvPts + dlPts
    + productProfileScore + priceListScore + catalogScore,
  ));

  if (!qualifiesForGoScore(text, tender.cpvCodes ?? [], priceListArticle, cpvMatchesEquipment, catalogStrong)) {
  ...
  return {
    ...
    breakdown: {
      keywordScore, foodFacilityScore: foodFacilityBoost, budgetScore, regionScore, industryScore,
      productProfileScore, priceListScore, catalogScore, cpvScore: cpvPts, deadlineScore: dlPts, matchedKeywords,
      priceListMatches: priceListHit.matched,
      topCatalogArticles: catalogHit.topArticles,
      matchedCatalogLines: catalogHit.matchedLines,
      matchedPortfolioSegments: portfolioHits.slice(0, 3),
