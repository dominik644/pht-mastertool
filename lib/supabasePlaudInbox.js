...
export async function insertSupabasePlaudNote(note) {
  const cfg = getConfig();
  if (!cfg) return { ok: false, skipped: true, error: 'supabase-env-missing' };
  const body = {
    id: note.id,
  ...
  try {
    ...
    lastError = (await res.text()).slice(0, 180);
  } catch (err) {
    lastError = err instanceof Error ? err.message : 'plaud_inbox-failed';
  }

  const fallbackRes = await fetch(`${cfg.url}/rest/v1/snapaddy_inbox`, {
    method: 'POST',
    headers: headers(cfg.key, { Prefer: 'return=minimal' }),
    body: JSON.stringify(body),
    signal: AbortSignal.timeout(12_000),
  });
  if (fallbackRes.ok) return { ok: true, table: 'snapaddy_inbox' };
  const fallbackText = await fallbackRes.text();
  return {
    ok: false,
    error: `fallback ${fallbackRes.status}: ${fallbackText.slice(0, 180)} | plaud_inbox: ${lastError || ''}`,
  };
}
...
const OAUTH_ROW_ID = '__plaud_oauth__';

async function fetchOauthFromTable(table) {
  const cfg = getConfig();
  if (!cfg) return null;
  try {
    const params = new URLSearchParams({
    ...
    const token = String(payload?.refresh_token || '').trim();
    return token || null;
  } catch {
    return null;
  }
...
export async function loadPlaudRefreshToken() {
  return (await fetchOauthFromTable('plaud_inbox'))
    || (await fetchOauthFromTable('snapaddy_inbox'));
}

export async function savePlaudRefreshToken(refreshToken) {
  const cfg = getConfig();
  const token = String(refreshToken || '').trim();
  if (!cfg || !token) return false;
  const body = {
    id: OAUTH_ROW_ID,
    payload: { id: OAUTH_ROW_ID, kind: 'oauth', refresh_token: token },
    status: 'dismissed',
    received_at: new Date().toISOString(),
  };
  for (const table of ['plaud_inbox', 'snapaddy_inbox']) {
    try {
