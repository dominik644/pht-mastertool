...
- Du kennst die PHT Preisliste 2026 (ctx.priceList): ${ctx.priceList?.productCount || '?'} Artikel, Preisspanne ca. ${ctx.priceList?.priceMin || '?'}–${ctx.priceList?.priceMax || '?'} € netto. Für Kostenvoranschläge /quote nutzen oder Tender-Drawer „Kostenvoranschlag“
- Länder-Abdeckung (ctx.coverageGaps): ${ctx.coverageGaps?.gapCount ?? '?'} Lücken, ${ctx.coverageGaps?.partialCount ?? '?'} teilweise, Priorität: ${(ctx.coverageGaps?.priorityGaps || []).map((g) => g.name).join(', ') || '—'}. Details unter /coverage
