    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          path: {
            type: 'string',
            enum: ['/', '/plan', '/command', '/dashboard', '/pipeline', '/tenders', '/watchlist', '/calendar', '/todo', '/go-no-go', '/workflow', '/alerts', '/analytics', '/quote', '/coverage', '/opportunities'],
          },
        },
        required: ['path'],
      },
...
- Du kennst die PHT Preisliste 2026 (ctx.priceList): ${ctx.priceList?.productCount || '?'} Artikel in ${ctx.priceList?.categoryCount || '?'} Kategorien, Preisspanne ca. ${ctx.priceList?.priceMin || '?'}–${ctx.priceList?.priceMax || '?'} € netto. Top-Kategorien: ${(ctx.priceList?.topCategories || []).slice(0, 5).map((c) => c.name).join('; ') || '—'}. Für Kostenvoranschläge /quote oder Tender-Drawer „Kostenvoranschlag“
- Live-Quellen: ${ctx.dataSource || 'lädt…'}
- Länder-Abdeckung (ctx.coverageGaps): ${ctx.coverageGaps?.gapCount ?? '?'} Lücken, ${ctx.coverageGaps?.partialCount ?? '?'} teilweise, ${ctx.coverageGaps?.coveredCount ?? '?'} abgedeckt. Priorität: ${(ctx.coverageGaps?.priorityGaps || []).map((g) => g.name).join(', ') || '—'}. Details unter /coverage
...
function localReply(ctx, lastUserMsg) {
  ...
  if (q.includes('abdeckung') || q.includes('coverage') || q.includes('lücke') || q.includes('länder')) {
    const cg = ctx.coverageGaps || {};
    const prio = (cg.priorityGaps || []).map((g) => `• **${g.name}** (${g.tenderCount} Treffer) – ${g.nextStep}`).join('\n');
    return [
      '**Länder-Abdeckung (Vergabeportale):**',
      `• Abgedeckt: ${cg.coveredCount ?? '?'}`,
      `• Teilweise: ${cg.partialCount ?? '?'}`,
      `• Lücken: ${cg.gapCount ?? '?'}`,
      '',
      prio ? `**Prioritäts-Lücken:**\n${prio}` : '',
      '',
      'Vollständige Karte: /coverage',
    ].filter(Boolean).join('\n');
  }

  if (q.includes('preisliste') || q.includes('angebot') || q.includes('kostenvoranschlag') || q.includes('kalkulation')) {
    const pl = ctx.priceList || {};
    const cats = (pl.topCategories || []).slice(0, 6).map(
      (c) => `• ${c.name}: ${c.productCount} Artikel, ${c.priceMin}–${c.priceMax} €`,
    ).join('\n');
    return [
      '**PHT Preisliste 2026 (AT/DE):**',
      `• ${pl.productCount || '?'} Artikel in ${pl.categoryCount || '?'} Kategorien`,
      `• Listenpreise netto: ca. ${pl.priceMin || '?'} – ${pl.priceMax || '?'} €`,
      '',
      '**Top-Kategorien:**',
      cats || '—',
      '',
      'Für Kostenvoranschläge: /quote oder im Tender-Drawer „Kostenvoranschlag“.',
      ctx.selectedTender ? `Aktueller Tender: „${ctx.selectedTender.title}“ – Produkte per Keyword-Match vorschlagen.` : '',
    ].filter(Boolean).join('\n');
  }
