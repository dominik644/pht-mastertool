...
import { looksLikeRawTranscript, unwrapPlaudText } from './plaudText.js';
...
function cleanNote(note) {
  ...
  const summary = unwrapPlaudText(note.summary);
  const transcriptRaw = unwrapPlaudText(note.transcript);
  const keepTranscript = transcriptRaw
    && !transcriptRaw.startsWith('{')
    && !looksLikeRawTranscript(transcriptRaw);
  return { ...note, summary, transcript: keepTranscript ? transcriptRaw : '' };
}
...
export function normalizePlaudPayload(rawBody) {
  ...
  return {
    title,
    transcript: unwrapPlaudText(transcript),
    summary: unwrapPlaudText(summary),
    actionItems,
    recordedAt,
...
export async function insertPlaudNote(normalized, { replace = false } = {}) {
  ...
  if (existing && !replace) return { ...existing, skipped: true };
  const note = {
    id,
    title: normalized.title || existing?.title || 'Plaud Note',
    transcript: normalized.transcript || '',
    summary: unwrapPlaudText(normalized.summary || ''),
    actionItems: Array.isArray(normalized.actionItems) ? normalized.actionItems : [],
    recordedAt: normalized.recordedAt || existing?.recordedAt || new Date().toISOString(),
...
export async function listPendingPlaudNotes() {
  ...
  return mergeNotes(fromSb, fromFile).map(cleanNote);
}
...
export async function fetchPlaudNoteRaw(id) {
  return readAll()[id] ?? (await fetchSupabasePlaudNote(id));
}

export async function fetchPlaudNote(id) {
  const note = await fetchPlaudNoteRaw(id);
  return note ? cleanNote(note) : note;
}
