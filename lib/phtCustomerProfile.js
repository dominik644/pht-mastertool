...
/** DACH + Südosteuropa (SEE) – Fokusregion Vertrieb Ost / Discovery */
export const FOCUS_COUNTRIES = ['AT', 'DE', 'CH', 'HU', 'SI', 'HR', 'SK', 'CZ', 'RO', 'BG'];
export const SEE_COUNTRIES = ['HU', 'SI', 'HR', 'SK', 'CZ', 'RO', 'BG'];
export const DACH_COUNTRIES = ['AT', 'DE', 'CH'];

/**
 * Kunden einem Vertriebsrep zuweisen (customer-priorities.json):
 * - `owner`: Legacy-Feld, z. B. „Dominik Weller“ (Standard für alle Excel-Importe)
 * - `salesRep`: optional – überschreibt owner für Territorium-Filter
 *   Werte: „Dominik Weller“ | „Vertrieb Ost“ | „Vertrieb West“ | Name des Kollegen
 * - Territorium „Vertrieb Ost“ = owner/salesRep „Dominik Weller“ + Ost-Bundesländer (Wien, NÖ, OÖ, STM, Bgld, Ktn)
 * - Territorium „Vertrieb West“ = salesRep „Vertrieb West“ + Vorarlberg, Tirol, Salzburg (Platzhalter)
 * Beispiel JSON: { "name": "…", "owner": "Dominik Weller", "salesRep": "Vertrieb West", "bundesland": "Tirol" }
 */
export const PHT_CUSTOMER_PROFILE = {
  version: '2026-07-06-v2',
  owner: 'Dominik Weller',
  defaultSalesRep: 'Dominik Weller',
  defaultTerritory: 'Vertrieb Ost',
  region: 'AT Ost / DACH / SEE (HU, SI, HR, SK, CZ, RO, BG)',
  ...
  targetIndustries: [
    ...
    { id: 'microfarming', label: 'Microfarming / Vertical Farming / Indoor-Anbau', priority: 'A', rationale: 'Controlled Environment Agriculture, Hygiene bei Ernte/Verarbeitung' },
    ...
    { id: 'ingredients', label: 'Zutaten / Aromen / Stärke / Gewürze / Spice', priority: 'B', rationale: 'Industrielle Verarbeitung, Behälterwasch, Gewürzmühlen' },
...
export const SECTOR_RULES = [
  { id: 'microfarming', label: 'Microfarming / Vertical Farming', kw: /vertical.?farm|indoor.?farm|micro.?farm|hydropon|aeropon|controlled.?environment|cea\b|urban.?farm|plant.?factory|infarm|gotham.?greens|bowery/i, growth: true, priorityA: true },
  { id: 'insects', label: 'Insekten / Alternative Protein', kw: /insekt|insect|käfer|kaefer|mealworm|mehlwurm|grillen|cricket|larven|entomo|bug.?farm|breeding|bsf|black.?soldier|hermetia|tebrio|protix|ynsect|essento|snack.?insect|entoprotech|insectum/i, growth: true, priorityA: true },
  ...
  { id: 'beverage', label: 'Getränke / Saft / Juice', kw: /getränk|getraenk|saft|juice|brauerei|brewery|mineral|fruchtsaft|cola|limonade|bier|energy|soft.?drink|wasserabfüll|wasserabfuell|eistee|kombucha|nektar|smoothie|fructal|rauch.?saft|hell.?energy|oshee/i, growth: true },
  ...
  { id: 'ingredients', label: 'Zutaten / Aromen / Gewürze', kw: /aroma|esarom|gewürz|gewuerz|spice|ingredient|extrakt|stärke|starke|zucker|mühle|muehle|hefe|backhefe|gelatine|lecithin|emulgator|stabilisator|süßstoff|suessstoff|podravka|vitana|transavia|würzmittel|wuerzmittel/i },
...
export function computePotentialScore({ sector, excelScore = 10, status = {}, exchangeHints = [], country, isMeat, potentialScore }) {
  ...
  if (country === 'AT') score += 8;
  if (DACH_COUNTRIES.includes(country) && country !== 'AT') score += 4;
  if (SEE_COUNTRIES.includes(country)) score += 3;
...
export function assignPriority({ sector, isMeat, potentialScore, excelAbc, status = {} }) {
  if (isMeat) return 'C';
  const priorityASectors = ['vegan', 'bio', 'convenience', 'pharma', 'babyfood', 'insects', 'plant_based', 'frozen', 'microfarming'];
  if (
    sector.growth &&
